    ...
    const cases = [
        ...
        },
        {
            ...
            class Calculator<N extends number> {
                add(a: N, b: N) { return a + b }
                subtract(a: N, b: N) { return a - b }
                multiply(a: N, b: N) { return a * b }
            }`,
            output: trim`//
            class Calculator<N extends number> {
        ...
        {
            note: 'rec.query.ts fixture',
            input: trim`//

            import type { RecColl } from "./rec.coll.ts"

            /**
             * The RecQuery class for querying and performing mutations on records.
             */
            export class RecQuery<Tb extends string = string> {

                readonly coll: RecColl<Tb>

                constructor(opts: {}, coll: RecColl<Tb>) {
                    this.coll = coll
                }
                 ...
                 * @param params
                 */
                async query(sql: string, params?: Record<string, unknown>): Promise<void> {
                }
